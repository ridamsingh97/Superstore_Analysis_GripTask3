...
```{r}
aa <- ggplot(superstore,aes(x=Sales,fill=Sales)) + geom_histogram(binwidth = 600) + ggtitle("Sales") + xlim(0,5000) + ylim(0,2000)
ab <- ggplot(superstore,aes(x=Sales)) + geom_boxplot()

```

```{r}
ac <- ggplot(superstore,aes(x=Discount)) + geom_histogram(aes(fill='clarity')) + ggtitle("Discount")
ad<-ggplot(superstore,aes(x=Discount)) + geom_boxplot()
```


```{r}
ae  <- ggplot(superstore,aes(x=Profit)) + geom_histogram(aes(fill='clarity')) + ggtitle("Profit")  + xlim(-1500,2000)
af  <- ggplot(superstore,aes(x=Profit)) + geom_boxplot()
```
...
Now moving our focus to the Catogories sold by the store 
